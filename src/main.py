...
class Game(DirectObject):
  ...
  def __init__(self):
    ...
    # parentnode.reparentTo(boxNP) # inherit transforms
    #     # the camera
    ...
    # self.gameNP.setHpr(1, 1, 30)
    
    
    #load environ model
    self.environ = loader.loadModel("../models/testenvironement")
    ...
    # Box (that we control)
    shape = BulletBoxShape(Vec3(0.5, 1, 0.5))

    self.boxNP = self.gameNP.attachNewNode(BulletRigidBodyNode('Box'))
  ...
  def movealongtrack(self):
    ...
    self.lerp = LerpPosInterval(self.gameNP,
                    duration,
                    pos,
                    other = None,
                    blendType='noBlend',
  ...
  #   trackpath = Mopath.Mopath()
  #   trackpath.loadFile('../models/trackcurve')
  #   trackInterval = MopathInterval(trackpath, gamenode, 'track')
  #   trackInterval.start
  ...
  def setup(self):
    ...
    # self.parentnode.reparentTo(boxNP) # inherit transforms
